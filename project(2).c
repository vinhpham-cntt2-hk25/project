...
void bookTicket() { // Chuc nang dat ve
    ...
    if (ticketCount >= MAXTICKETS) {
        printf("Khong the dat them ve: danh sach ve da day.\n");
        return;
    }
    ...
    for (int i = 0; i < ticketCount; ++i) {
        printf("Ma ve: %s | Ma chuyen xe: %s | Ten khach: %s | SDT: %s | Ghe: %d | Gia: %.2f | Ngay dat: %s | Trang thai: %s\n",
            ...
            tickets[i].paymentStatus == 1 ? "Da thanh toan" : (tickets[i].paymentStatus == -1 ? "Vo hieu hoa" : "Chua thanh toan"));
    }
}
